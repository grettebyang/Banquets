...
function TableSetDown(){
	...
	if(activate == noone) {
		...
		with(activate) {
			if(global.iHolding.entityPickup && cartItem == object_get_parent(global.iHolding.object_index) && itemCount < cartAmount) {
				tableList[global.iHolding.tableTypeIndex]++;
				itemCount++;
	...
	else if(activate.entityActivateArgs[0] == global.iHolding.object_index) // if placing a table back into a table spawner
	{
		instance_destroy(global.iHolding);
		global.iHolding = noone;
	}
