...
function ActivateGasparoQuest(foodName, npcName, giveFood, giveLinen){
	...
	switch (global.questStatus[? gasparo_obj])
	{
		case 0: //not started
		{
			...
			if(giveFood)
			{
				...
				global.questStatus[? gasparo_obj] = 2;
			...
			else if (hasFood)
			{
				...
				global.questStatus[? gasparo_obj] = 1;
			
			}
			else
			{
				//offer quest
				NewTextBox("Hi. My name is Gasparo.");
				NewTextBox("And.");
		...
		case 1: //quest in progress
		{
			...
			{
				NewTextBox("*Sniff* *sniff* *sniff* *sniff* *sniff* *sniff* Did you bring that for me?", 0);
			}
			else
			{
				//clue reminder
		...
		case 2: // dog introduces linen quest
		{
			if(giveLinen)
			{
				...
				global.questStatus[? gasparo_obj] = 4;
			...
			else if (hasLinen)
			{
				...
				global.questStatus[? gasparo_obj] = 3;
			}
			else
			{
				//clue reminder
