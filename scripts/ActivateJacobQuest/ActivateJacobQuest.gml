...
function ActivateJacobQuest(foodName, npcName, giveFood, giveLinen){
	...
	switch (global.questStatus[? jacob_obj])
	{
		case 0: //not started
		{
			...
			if(giveFood)
			{
				...
				global.questStatus[? jacob_obj] = 2;
			...
			else if (hasFood)
			{
				...
				global.questStatus[? jacob_obj] = 1;
			
			}
			else
			{
				//offer quest
				NewTextBox("My name's Jacob.");
		...
		case 2: // dog introduces linen quest
		{
			if(giveLinen)
			{
				...
				global.questStatus[? jacob_obj] = 4;
			...
			else if (hasLinen)
			{
				...
				global.questStatus[? jacob_obj] = 3;
			}
			else
			{
				//clue reminder
