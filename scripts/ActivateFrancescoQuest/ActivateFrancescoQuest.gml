...
function ActivateFrancescoQuest(foodName, npcName, giveFood, giveLinen){
	...
	switch (global.questStatus[? francesco_obj])
	{
		case 0: //not started
		{
			...
			if(giveFood)
			{
				...
				global.questStatus[? francesco_obj] = 2;
			...
			else if (hasFood)
			{
				...
				global.questStatus[? francesco_obj] = 1;
			
			}
			else
			{
				//offer quest
				NewTextBox("Hello, have you seen my brother around? His name is Omobono. He went out to look for some Chicken alfredo.");
				NewTextBox("If you see him around, tell him Francesco is looking for him.");
				global.questStatus[? francesco_obj] = 1;
			}
		}break;
		...
		case 2: // dog introduces linen quest
		{
			if(giveLinen)
			{
				...
				global.questStatus[? francesco_obj] = 4;
			...
			else if (hasLinen)
			{
				...
				global.questStatus[? francesco_obj] = 3;
			}
			else
			{
				//clue reminder
