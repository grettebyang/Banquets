...
function ActivateDavidQuest(foodName, npcName, giveFood, giveLinen){
	...
	switch (global.questStatus[? david_obj])
	{
		case 0: //not started
		{
			...
			if(giveFood)
			{
				...
				global.questStatus[? david_obj] = 2;
			...
			else if (hasFood)
			{
				...
				global.questStatus[? david_obj] = 1;
			
			}
			else
			{
				//offer quest
				NewTextBox("You're quite small for a human, are you sure you're old enough to be working here?");
				NewTextBox("Well, I suppose you know more than me. Anyway, I'm David.");
				NewTextBox("What am I doing out here? I'm looking for some food to bring to Dogsgiving.");
				NewTextBox(foodName + " in particular.");
				NewTextBox("If you happen to come across any, please bring them to me.");
				global.questStatus[? david_obj] = 1;
			}
		}break;
		...
		case 1: //quest in progress
		{
			...
			{
				//clue reminder
				NewTextBox(foodName + " aren't too common. They sometimes appear in the kitchen on the fifth floor on event days.", 0);
			}
		...
		case 2: // dog introduces linen quest
		{
			if(giveLinen)
			{
				...
				global.questStatus[? david_obj] = 4;
			...
			else if (hasLinen)
			{
				...
				global.questStatus[? david_obj] = 3;
			}
			else
			{
				//clue reminder
