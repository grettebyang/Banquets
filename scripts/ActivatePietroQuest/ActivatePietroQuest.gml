...
function ActivatePietroQuest(foodName, npcName, giveFood, giveLinen){
	...
	switch (global.questStatus[? pietro_obj])
	{
		case 0: //not started
		{
			...
			if(giveFood)
			{
				...
				global.questStatus[? pietro_obj] = 2;
			...
			else if (hasFood)
			{
				...
				global.questStatus[? pietro_obj] = 1;
			
			}
			else
			{
				//offer quest
				NewTextBox("I'm Pietro.");
				NewTextBox("I would do anything for " + foodName, 0);
				NewTextBox("In fact, I usually have all kinds of food.");
				NewTextBox("Problem is, I can't get back like this and I still need to get some fresh " + foodName + " to bring back for Dogsgiving.");
				NewTextBox("If you can help me find some, I'll show you my stash of goods I've got back at our space.");
				global.questStatus[? pietro_obj] = 1;
			}
		}break;
		...
		case 2: // dog introduces linen quest
		{
			if(giveLinen)
			{
				...
				global.questStatus[? pietro_obj] = 4;
			...
			else if (hasLinen)
			{
				...
				global.questStatus[? pietro_obj] = 3;
			}
			else
			{
				//clue reminder
