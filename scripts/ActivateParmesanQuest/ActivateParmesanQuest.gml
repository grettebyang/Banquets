...
function ActivateParmesanQuest(foodName, npcName, giveFood, giveLinen){
	...
	switch (global.questStatus[? parmesan_obj])
	{
		case 0: //not started
		{
			...
			if(giveFood)
			{
				...
				global.questStatus[? parmesan_obj] = 2;
			...
			else if (hasFood)
			{
				...
				global.questStatus[? parmesan_obj] = 1;
		...
		case 2: // dog introduces linen quest
		{
			if(giveLinen)
			{
				...
				global.questStatus[? parmesan_obj] = 4;
			...
			else if (hasLinen)
			{
				...
				global.questStatus[? parmesan_obj] = 3;
			}
			else
			{
				//clue reminder
		...
		{
			NewTextBox("Thanks again! I'm off to find everyone!\nGoodbye!",0,[],ReloadRoom);	
		} break;
		default: break;
	}
}