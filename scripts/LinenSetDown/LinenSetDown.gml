...
function LinenSetDown(){
	...
	if(activate == noone) {
	...
	else if(object_get_parent(activate.object_index) == table_obj && (!activate.tableLinen || (activate.canFluff && activate.tableFluff == -1)) && activate.tableEmpty) { // if placing linen on table
		SpreadLinen(global.iHolding.image_index);
		instance_destroy(global.iHolding);
		global.iHolding = noone;
	}
	else if(object_get_parent(activate.object_index) == genCart_obj) { // if active item is a cart
		if(global.iHolding.image_index < 2)
		{
			with(activate) {
				if(global.iHolding.entityPickup && cartItem == global.iHolding.object_index && itemCount < cartAmount) {
					linenColorIndex = global.iHolding.image_index;
		...
		else if(activate.object_index == laundryBin_obj)
		{
			with(activate) {
				if(global.iHolding.entityPickup && cartItem == global.iHolding.object_index && itemCount < cartAmount) {
					itemCount++;
	...
	else if(activate.entityActivateArgs[0] == linen_obj && activate.image_index == global.iHolding.image_index) // if placing a chair back into a chair spawner
	{
		instance_destroy(global.iHolding);
		global.iHolding = noone;
	}
