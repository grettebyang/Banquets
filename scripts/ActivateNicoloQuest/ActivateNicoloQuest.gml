...
function ActivateNicoloQuest(foodName, npcName, giveFood, giveLinen){
	...
	switch (global.questStatus[? nicolo_obj])
	{
		case 0: //not started
		{
			...
			if(giveFood)
			{
				...
				global.questStatus[? nicolo_obj] = 2;
			...
			else if (hasFood)
			{
				...
				global.questStatus[? nicolo_obj] = 1;
			
			}
			else
			{
				//offer quest
				NewTextBox("I'm Nicolo.");
		...
		case 2: // dog introduces linen quest
		{
			if(giveLinen)
			{
				...
				global.questStatus[? nicolo_obj] = 4;
			...
			else if (hasLinen)
			{
				...
				global.questStatus[? nicolo_obj] = 3;
			}
			else
			{
				//clue reminder
